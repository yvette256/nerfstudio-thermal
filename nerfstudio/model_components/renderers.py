...
from nerfstudio.utils.rgbt_utils import rgb_to_rgbt_image
...
BackgroundColorRGBT = Union[Literal["random", "last_sample", "black", "white"], Float[Tensor, "4"], Float[Tensor, "*bs 4"]]
BACKGROUND_COLOR_RGBT_OVERRIDE: Optional[Float[Tensor, "4"]] = None
...
@contextlib.contextmanager
def background_color_rgbt_override_context(mode: Float[Tensor, "4"]) -> Generator[None, None, None]:
    """Context manager for setting background mode."""
    global BACKGROUND_COLOR_RGBT_OVERRIDE
    old_background_color = BACKGROUND_COLOR_RGBT_OVERRIDE
    try:
        BACKGROUND_COLOR_RGBT_OVERRIDE = mode
        yield
    finally:
        BACKGROUND_COLOR_RGBT_OVERRIDE = old_background_color
...
class RGBTRenderer(nn.Module):
    """RGBT volumetric rendering.

    Args:
        background_color: Background color as RGBT. Uses random colors if None.
    """

    def __init__(self, background_color: BackgroundColorRGBT = "random") -> None:
        super().__init__()
        self.background_color: BackgroundColorRGBT = background_color

    @classmethod
    def combine_rgb(
            cls,
            rgb: Float[Tensor, "*bs num_samples 4"],
            weights: Float[Tensor, "*bs num_samples 1"],
            background_color: BackgroundColorRGBT = "random",
            ray_indices: Optional[Int[Tensor, "num_samples"]] = None,
            num_rays: Optional[int] = None,
    ) -> Float[Tensor, "*bs 4"]:
        """Composite samples along ray and render color image.
        ...
        Args:
            rgb: RGBT for each sample
            weights: Weights for each sample
            background_color: Background color as RGBT.
            ray_indices: Ray index for each sample, used when samples are packed.
            num_rays: Number of rays, used when samples are packed.
    ...
    @classmethod
    def get_background_color(
            cls, background_color: BackgroundColorRGBT, shape: Tuple[int, ...], device: torch.device
    ) -> Union[Float[Tensor, "4"], Float[Tensor, "*bs 4"]]:
        """Returns the RGBT background color for a specified background color.
        Note:
            This function CANNOT be called for background_color being either "last_sample" or "random".
        ...
        Returns:
            Background color as RGBT.
        """
        assert background_color not in {"last_sample", "random"}
        assert shape[-1] == 4, "Background color must be RGBT."
        if BACKGROUND_COLOR_RGBT_OVERRIDE is not None:
            background_color = BACKGROUND_COLOR_RGBT_OVERRIDE
        if isinstance(background_color, str) and background_color in colors.COLORS_RGBT_DICT:
            background_color = colors.COLORS_RGBT_DICT[background_color]
        assert isinstance(background_color, Tensor)
    ...
    def blend_background(
        ...
        """
        if image.size(-1) < 4:
            return rgb_to_rgbt_image(image, is_thermal)

        rgb, opacity = image[..., :3], image[..., 3:]
        rgbt = rgb_to_rgbt_image(image, is_thermal)
        if background_color is None:
            background_color = self.background_color
            if background_color in {"last_sample", "random"}:
                background_color = "black"
        background_color = self.get_background_color(background_color, shape=rgbt.shape, device=rgbt.device)
        assert isinstance(background_color, torch.Tensor)
        return rgbt * opacity + background_color.to(rgbt.device) * (1 - opacity)
    ...
    def blend_background_for_loss_computation(
        ...
        Args:
            gt_image: The ground truth image.
            pred_image: The predicted RGBT values (without background blending).
            pred_accumulation: The predicted opacity/ accumulation.
            is_thermal: 0 if RGB, 1 if thermal, per pixel.
        Returns:
            A tuple of the predicted and ground truth RGB values.
        ...
        if background_color == "last_sample":
            ...
            background_color = torch.rand_like(pred_image)
            pred_image = pred_image + background_color * (1.0 - pred_accumulation)
        gt_image = self.blend_background(gt_image, is_thermal, background_color=background_color)
        return pred_image, gt_image
